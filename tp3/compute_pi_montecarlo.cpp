...
// //test11
// void test_pi_computation(unsigned long nb_points, unsigned int nb_threads)
// {
//     FloatKind error{std::abs(pi - compute_pi(nb_points, nb_threads))};
...
FloatKind compute_pi(unsigned long nb_points, unsigned int nb_threads)
{
    ...
    for (int i = 0; i < nb_threads; i++)
    {
        ...
        pool.push_back(std::thread(std::move(task), nb_points / nb_threads));
    }

    unsigned long points_inside = 0;
    for (int i = 0; i < nb_threads; i++)
    {
        pool[i].join();
        points_inside = points_inside + results[i].get();
    }

    FloatKind inside_circle = static_cast<FloatKind>(points_inside);
    FloatKind total = static_cast<FloatKind>(nb_points);
    return 4 * inside_circle / total;
}

// test2
// With nb_points = 70000000 and -o, execution time is ~10s, much faster than single-threaded computations
void test_multithreaded_pi_computation(unsigned long nb_points, unsigned int nb_threads)
{
    FloatKind error{std::abs(pi - compute_pi(nb_points, nb_threads))};
    std::cout << "Error is: " << error << "\n\n";
}
...
int main(int argc, char *argv[])
{
    ...
    unsigned int nb_threads = std::stoull(argv[2]);
    // test_pi_computation(nb_points);
    // test_execution_time(nb_points);
    // test_variability(nb_points);
    test_multithreaded_pi_computation(nb_points, nb_threads);
};